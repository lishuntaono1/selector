...
//
func ParseXPathExpression(xpathExpression string) AstNode {
	p := &Parser{s: newScanner(xpathExpression)}
	if n := p.parseExpression(nil); p.s.kind != LexEof {
		panic(fmt.Sprintf("%s has an invalid token.", xpathExpression))
	} else {
		return n
	}
}

func ParseXPathPattern(xpathPattern string) AstNode {
	p := &Parser{s: newScanner(xpathPattern)}
	if n := p.parsePattern(); p.s.kind != LexEof {
		panic(fmt.Sprintf("%s has an invalid token.", xpathPattern))
	} else {
		return n
	}
}
